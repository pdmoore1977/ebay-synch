...
class WPL_Model {
	...
	function handleResponse( $res, $is_second_request = false )	{
		...
		foreach ($res->getErrors() as $error) {
			...
			// Invalid license on GetOrders call
			if ( $error->getErrorCode() == '5822022' ) {
				update_option('wplister_license_error', 1 );
			}
			...
			if ( is_admin() && ! $this->is_ajax() && ! $this->is_rest() ) {
				$type = ( $error->SeverityCode == 'Error') ? 'error' : 'info';
				$htmlError = '<b>' . $error->SeverityCode . ': ' . $shortMessage . '</b>' . ' (#'  . $error->getErrorCode() . ') ';
				$htmlError .= '<br>' . $longMessage;

				// handle optional ErrorParameters
				if ( ! empty( $error->ErrorParameters ) ) {
					foreach ( $error->ErrorParameters as $param ) {
						$htmlError .= '<br><code>' . $param . '</code>';
					}
				}
				wple_show_message( $htmlError, $type, ['persistent'=>true]);
				//echo $htmlMsg;
			}

...
class ListingsTable extends WP_List_Table {
        ...
        if ( ProductWrapper::hasVariations( $item['post_id'] ) ) {
            ...
            if ( isset( $profile_data['details']['variations_mode'] ) && ( $profile_data['details']['variations_mode'] == 'flat' ) ) {
                ...
                if ( @$profile_data['details']['shipping_service_type'] == 'calc' ) {
                    $variations_html .= '<p>';
                    $variations_html .= __( 'Notice: eBay does not support individual weight and dimensions for variations when using calculated shipping services.', 'wp-lister-for-ebay' );
                    $variations_html .= '</p>';
                }

                // display split variations button
                if ( $item['status'] == 'prepared' )
                    $variations_html .= sprintf('<a href="?page=%s&action=%s&auction=%s&_wpnonce=%s" class="button">%s</a>',sanitize_key($_REQUEST['page']),'wple_split_variations',$item['id'], wp_create_nonce( 'wplister_split_variations' ), __( 'Split variations into single items', 'wp-lister-for-ebay' ) );
                ## END PRO ##
    ...
    function column_quantity($item){
        ...
        if ( isset( $profile_details['quantity'] ) && ( $profile_details['quantity'] > 0 ) ) {
            if ( get_option( 'wplister_handle_stock' ) == 1 ) {
                // $img_url  = WPLE_PLUGIN_URL . '/img/warning.png';
                $img_url  = WPLE_PLUGIN_URL . 'img/error.gif';

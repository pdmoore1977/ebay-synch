...
class EbayOrdersTable extends WP_List_Table {
    ...
    function column_total($item){
        ...
        /*** ## BEGIN PRO ## ***/
        // show warning if the number of purchased units is different from the total stock units processed #30385
        // this warning will be appended to $tipmsgs and $icon will be overwritten with error.gif
        // Active orders will not be checked by this method
        if ( $wpl_order = WPL_InventoryCheck::checkThisOrderForStockReductionsMismatch( $item['id'], true ) ) {
            $tipmsgs .= '<br><b>Warning:<br>';
            $tipmsgs .= sprintf( 'Stock reduction mismatch. The number of purchased units (%s) is different from the total stock units processed (%s).', $wpl_order['units_purchased'], $wpl_order['stock_units_reduced'] ) . '</b><br>';
            $img_url  = WPLE_PLUGIN_URL . 'img/error.gif';
            if ( $wpl_order['line_items_mismatch'] ) $tipmsgs .= '<b>Note: The number of order line items do not match!</b><br>';
            $icon     = '&nbsp;<img src="'.$img_url.'" style="height:12px; padding:0;" class="tips" data-tip="'.$tipmsgs.'"/>';                
        }
        /*** ## END PRO ## ***/
    ...
    function column_PaymentMethod($item){
        ...
        switch( $item['eBayPaymentStatus'] ){
            case 'NoPaymentFailure':
                ...
                if ( $item['CheckoutStatus'] == 'Incomplete' ) {
	                $value = __( 'Payment incomplete', 'wp-lister-for-ebay' );
                }
                break;
    ...
    function get_views(){
        ...
        $views['has_wc_order'] = "<a href='{$has_wc_order_url}' {$class} >".__( 'In WooCommerce', 'wp-lister-for-ebay' )."</a>";
        ...
        $views['has_no_wc_order'] = "<a href='{$has_no_wc_order_url}' {$class} >".__( 'Not in WooCommerce', 'wp-lister-for-ebay' )."</a>";

...
class WPLE_Update_API {
	 ...
	 * @var string
	 */
	public $version = WPLE_PLUGIN_VERSION;
	 ...
	 */
	public $text_domain = 'wp-lister-for-ebay';
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, __( 'Cheatin&#8217; huh?', 'wp-lister-for-ebay' ), '1.2' );
	}
	...
	private function init_properties() {
        ...
        $this->ame_menu_tab_activation_title 	= __( 'License Activation', 'wp-lister-for-ebay' );
        $this->ame_menu_tab_deactivation_title 	= __( 'License Deactivation', 'wp-lister-for-ebay' );
        ...
        $this->license_key 				= wple_get_license_key();
        $this->license_email 			= wple_get_license_email();
	 ...
	 */
	public function key() {
	    require_once WPLISTER_PATH .'/includes/am/classes/class-wc-key-api.php';
	...
	public function check_for_new_version( $quiet = false ) {
		...
		if ( is_object( $response ) && !empty( $response ) && $response->new_version ) {
			...
			update_option( 'wple_update_details', $response, false );
			delete_option( 'wplister_update_details' );
			...
			$update->new_version  = WPLE_PLUGIN_VERSION;
			$update->title        = 'WP-Lister for eBay';
			$update->upgrade_html = '';
			update_option( 'wple_update_details', $update, false );
			delete_option( 'wplister_update_details' );
	...
	public function on_plugin_activation() {
		...
		// reset instance ID
		$instance_key = str_replace( array('http://','https://','www.'), 'wple.', get_site_url() ); // example.com
        // save new instance
	...
	public static function license_inactive_notice() { 
		...
		$error = sprintf( __( 'The plugin license key for <b>WP-Lister Pro for eBay</b> is currently deactivated so you won\'t be able to get updates and support! %sClick here%s to activate your license', 'wp-lister-for-ebay' ), '<a href="' . esc_url( admin_url( 'admin.php?page=wplister-settings&tab=license' ) ) . '">', '</a>' );

        ## BEGIN PRO ##
        //$error = sprintf( __( '<h1>Order Sync Disabled!</h1><p>Your WP-Lister license key is currently deactivated. Please activate your license to continue getting order updates from eBay.</p><p><a href="%s">Click here</a> to activate your license.</p>', 'wp-lister-for-ebay' ), esc_url( admin_url( 'admin.php?page=wplister-settings&tab=license' ) ) );
        ## END PRO ##
	...
	public function check_external_blocking() {
		...
		if( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL === true ) {
			...
			if( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
				?>
				<div class="error">
					<p><?php printf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %s updates. Please add %s to %s.', 'wp-lister-for-ebay' ), $this->ame_software_product_id, '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>'); ?></p>
				</div>
				<?php
			}
